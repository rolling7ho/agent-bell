...
| The display is being recorded, shared, or mirrored | Turnring uses public ScreenCaptureKit and CoreGraphics state to defer native submission, plays one generic fallback chime, and keeps retrying until capture clears. A request is acknowledged only after Notification Center retains it outside capture. | Display-capture delivery policy tests; native outbox confirmation path |
...
| Automatic history cleanup sees an old agent timestamp shortly after a delayed alert | It removes the local dashboard item but leaves the newly delivered Notification Center item intact. Explicit user clears still remove both. | Notification cleanup separation |
