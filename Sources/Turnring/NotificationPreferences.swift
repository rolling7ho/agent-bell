...
final class NotificationPreferences {
    private enum Key {
        ...
        static func integration(_ provider: AgentProvider) -> String {
            "settings.integrations.\(provider.rawValue).enabled"
        }
    ...
    func notificationsEnabled(for surface: NotificationSurface) -> Bool {
        guard surface.isSupported else { return false }
    ...
    var minimumTaskDuration: TimeInterval {
        NotificationSurface.supportedCases
            .map(minimumTaskDuration)
            .max() ?? 0
    }

    func setMinimumTaskDuration(_ duration: TimeInterval) {
        for surface in NotificationSurface.supportedCases {
            setMinimumTaskDuration(duration, for: surface)
        }
    }
    ...
    var localPreviewCharacterLimit: Int {
        get {
            guard defaults.object(forKey: Key.localPreviewCharacterLimit) != nil
                ...
                return AttentionPreviewFormatter.maximumCharacters
            }
            return max(
                10,
                min(
    ...
    func integrationEnabled(for provider: AgentProvider) -> Bool {
        defaults.bool(forKey: Key.integration(provider))
    }

    func setIntegrationEnabled(_ enabled: Bool, for provider: AgentProvider) {
        defaults.set(enabled, forKey: Key.integration(provider))
    }

    var selectedIntegrations: [AgentProvider] {
        AgentProvider.allCases.filter(integrationEnabled)
    }

    func setSelectedIntegrations(_ providers: [AgentProvider]) {
        let selected = Set(providers.map(\.rawValue))
        for provider in AgentProvider.allCases {
            setIntegrationEnabled(
                selected.contains(provider.rawValue),
                for: provider
            )
        }
    }
