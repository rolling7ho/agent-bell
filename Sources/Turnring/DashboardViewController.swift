...
final class DashboardViewController: NSViewController, NSTableViewDataSource, NSTableViewDelegate {
    ...
    private func buildInterface() {
        ...
        let title = NSTextField(labelWithString: "Turnring")
        title.font = .systemFont(ofSize: 20, weight: .semibold)

        let titleSymbol = NSImageView()
        titleSymbol.image = TurnringBrand.monochromeMark() ?? NSImage(
            systemSymbolName: "circle.circle",
            accessibilityDescription: "Turnring"
        )?.withSymbolConfiguration(
            NSImage.SymbolConfiguration(pointSize: 17, weight: .semibold)
        )
        titleSymbol.contentTintColor = .controlAccentColor
        titleSymbol.setAccessibilityLabel("Turnring")
        ...
        NSLayoutConstraint.activate([
            ...
            header.leadingAnchor.constraint(equalTo: contentRoot.leadingAnchor, constant: 16),
            header.trailingAnchor.constraint(equalTo: contentRoot.trailingAnchor, constant: -16),
            header.topAnchor.constraint(equalTo: contentRoot.topAnchor, constant: 14),
            headerTop.widthAnchor.constraint(equalTo: header.widthAnchor),

            scrollView.leadingAnchor.constraint(equalTo: contentRoot.leadingAnchor, constant: 8),
            scrollView.trailingAnchor.constraint(equalTo: contentRoot.trailingAnchor, constant: -8),
            ...
            footer.leadingAnchor.constraint(equalTo: contentRoot.leadingAnchor, constant: 12),
            footer.trailingAnchor.constraint(equalTo: contentRoot.trailingAnchor, constant: -12),
            footer.bottomAnchor.constraint(equalTo: contentRoot.bottomAnchor, constant: -10),
...
private final class SessionCellView: NSTableCellView {
    ...
    func configure(
        ...
        providerIcon.image = session.testDisplayName == "Turnring"
        ...
        previewLabel.textColor = includesPrivateDetails || session.isTest
            ? .secondaryLabelColor
            : .tertiaryLabelColor
        statusLabel.stringValue = session.statusDisplayName
    ...
    func animateFallingBackward(completion: @escaping () -> Void) {
        ...
        layer.add(group, forKey: "turnring-fall-back")
        CATransaction.commit()
    }
}

@MainActor
private final class RoundedRectButton: LiquidGlassButton {
    enum Emphasis {
        case secondary
        case primary
        case destructive
    ...
    init(title: String, emphasis: Emphasis = .secondary) {
        super.init(
            title: title,
            glassStyle: emphasis.glassStyle
        )
        configure()
    }

    init(symbol: String, accessibilityDescription: String) {
        super.init(title: "", glassStyle: .secondary)
        image = NSImage(
...
private extension RoundedRectButton.Emphasis {
