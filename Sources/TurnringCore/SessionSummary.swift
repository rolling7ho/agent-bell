...
public struct SessionSummary: Codable, Equatable, Sendable, Identifiable {
    ...
    public var formattedTitle: String {
        let safeTitle = TurnringSafeText.redacted(
    ...
    public var dashboardStateName: String {
        ...
        return "\(statusDisplayName) (\(formattedElapsedDuration))"
    }
    ...
    public var genericNotificationTitle: String {
        var value = "\(appDisplayName) • \(statusDisplayName)"
        if state == .finished,
           let formattedElapsedDuration
        {
    ...
    public var genericNotificationBody: String? {
        ...
        case "question", "elicitation_dialog":
            return "Open \(appDisplayName) to answer."
        case "plan_approval":
            return "Approve or reject the proposed plan."
        case "permission_request", "permission_prompt":
            return "Approve or deny the requested action."
        case "agent_needs_input":
            return "Open \(appDisplayName) to continue."
        case "rate_limit":
            return "Usage limit reached. Try again after it resets."
        case "max_output_tokens":
            return "The task reached its output limit."
        default:
            break
        }
        return switch state {
    ...
    public func dashboardPreview(
        ...
        {
            return TurnringSafeText.collapsed(
                contentPreview,
        ...
        case .working:
            "Task is running."
        case .attention:
            genericNotificationBody ?? "Needs your attention."
        case .finished:
            "Task finished."
        case .failed:
            genericNotificationBody ?? "Task stopped unexpectedly."
        case .ended:
            "Session ended."
        }
    }
    ...
    public var statusDisplayName: String {
        switch notificationType {
        case "question", "elicitation_dialog": "Question"
        case "plan_approval": "Plan approval"
        case "permission_request", "permission_prompt": "Permission"
        case "agent_needs_input": "Input needed"
        case "rate_limit": "Usage limit"
        case "max_output_tokens": "Output limit"
