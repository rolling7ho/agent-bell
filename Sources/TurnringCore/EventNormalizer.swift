...
public enum EventNormalizer {
    ...
    public static func normalize(
        ...
        guard let sessionID = payload["session_id"] as? String,
              TurnringValidation.isValidSessionID(sessionID)
        else {
            throw EventNormalizerError.invalidSessionID
        }

        guard let rawCWD = payload["cwd"] as? String,
              let cwd = TurnringValidation.normalizedAbsolutePath(rawCWD)
        else {
        ...
        case "Stop", "StopFailure":
            contentPreview = failureReason?.fallbackPreview
        ...
        case "Notification":
            contentPreview = AttentionPreviewFormatter.notificationPreview(
                provider: provider,
                origin: origin,
                notificationType: notificationType,
        ...
        return AgentEvent(
            ...
            turnID: turnID,
            cwd: cwd,
            projectName: TurnringValidation.projectName(for: cwd),
            timestamp: now,
            notificationType: sanitizedNotificationType(
                semanticNotificationType(
                    hookEventName: hookEventName,
                    notificationType: notificationType,
                    toolName: toolName,
    ...
    private static func sanitizedOptionalIdentifier(_ value: String?) -> String? {
        guard let value,
              TurnringValidation.isValidOptionalIdentifier(value)
        else {
            return nil
        }
        return value
    ...
    private static func semanticNotificationType(
        ...
        if let failureReason {
            return failureReason.rawValue
        }
        switch (hookEventName, toolName) {
        case ("PermissionRequest", _):
            return "permission_request"
        case ("PreToolUse", "request_user_input"),
             ("PreToolUse", "AskUserQuestion"):
            return "question"
        case ("PreToolUse", "ExitPlanMode"):
