...
public final class NavigationRouter: @unchecked Sendable {
    ...
    private func resumeDeadSession(_ session: SessionSummary) -> NavigationResult {
        guard AgentBellValidation.isValidSessionID(session.sessionID),
              let cwd = AgentBellValidation.validatedDirectory(session.cwd)
        else {
    ...
    private func openCodexDeepLink(_ sessionID: String) -> NavigationResult {
        guard AgentBellValidation.isValidSessionID(sessionID),
    ...
    private func sendVSCodeRequest(
        ...
        do {
            ...
            let requestURL = AgentBellPaths.focusRequestsDirectory
            ...
            guard let url = URL(string: "vscode://agentbell.focus/focus?request=\(requestID)"),
                  NSWorkspace.shared.open(url)
            else {
                try? fileManager.removeItem(at: requestURL)
                return .unavailable("VS Code did not accept the AgentBell focus request.")
            }
    ...
    private func resumeInGhostty(_ session: SessionSummary, cwd: String) -> Bool {
        guard let executable = validatedExecutable(for: session),
              let resumeArguments = AgentBellValidation.resumeArguments(
                  provider: session.provider,
                  sessionID: session.sessionID
              )
        else {
    ...
    private func createResumeScript(session: SessionSummary, cwd: String) throws -> URL {
        ...
        guard let arguments = AgentBellValidation.resumeArguments(
            provider: session.provider,
            sessionID: session.sessionID
        ) else {
            throw CocoaError(.fileReadInvalidFileName)
        }
        let scriptURL = AgentBellPaths.resumeScriptsDirectory
    ...
    private func validatedExecutable(for session: SessionSummary) -> String? {
        let candidates = [
            ...
            "\(AgentBellPaths.homeDirectory.path)/.local/bin/\(session.provider == .codex ? "codex" : "claude")",
            "\(AgentBellPaths.homeDirectory.path)/.npm-global/bin/\(session.provider == .codex ? "codex" : "claude")",
        ].compactMap { $0 }
        return candidates.first { path in
            AgentBellValidation.isValidExecutable(
                path,
